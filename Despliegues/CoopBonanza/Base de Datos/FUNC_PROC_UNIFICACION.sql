...
-- ----------------------------
--  Procedure definition for `bce_banco`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `bce_banco`;
CREATE PROCEDURE `bce_banco`() BEGIN DECLARE done INT DEFAULT 0;
DECLARE a CHAR(50);
...
IF NOT done THEN CALL set_bce_banco(a);
END IF;
UNTIL done END REPEAT;
CLOSE cur1;
END;
-- ----------------------------
--  Procedure definition for `bce_clientes`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `bce_clientes`;
CREATE PROCEDURE `bce_clientes`() BEGIN DECLARE done INT DEFAULT 0;
DECLARE a CHAR(50);
DECLARE cur1 CURSOR FOR 
SELECT 
  fld_id 
...
OPEN cur1;
REPEAT FETCH cur1 INTO a;
IF NOT done THEN CALL set_bce_cliente(a);
END IF;
UNTIL done END REPEAT;
CLOSE cur1;
END;
-- ----------------------------
--  Procedure definition for `bce_prestamos`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `bce_prestamos`;
CREATE PROCEDURE `bce_prestamos`() BEGIN DECLARE done INT DEFAULT 0;
DECLARE a CHAR(50);
DECLARE cur1 CURSOR FOR 
SELECT 
  fld_id_prestamos 
...
OPEN cur1;
REPEAT FETCH cur1 INTO a;
IF NOT done THEN CALL set_bce_prestamos(a);
END IF;
UNTIL done END REPEAT;
CLOSE cur1;
END;
-- ----------------------------
--  Procedure definition for `CargarDepositosLibreta`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `CargarRetirosLibreta`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `CargarRetirosLibreta`;
CREATE PROCEDURE `CargarRetirosLibreta`(
  in _CodSocio int, 
  in _NroCuenta varchar(8), 
  in _fecha date
) BeGIN 
SELECT 
  d.fecha, 
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_controles`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_controles`;
CREATE PROCEDURE `cont_actualizar_cta_controles`(
  IN `fechaDesde` date, IN `fechaHasta` date
) BEGIN DECLARE done INT DEFAULT 0;
DECLARE VarCuenta CHAR(50);
DECLARE VarGrupo CHAR(50);
DECLARE VarSubGrupo CHAR(50);
DECLARE VarSubCuenta CHAR(50);
DECLARE VarDebito DECIMAL(12, 2);
DECLARE VarCredito DECIMAL(12, 2);
DECLARE cur1 CURSOR FOR 
SELECT 
  LEFT(TransDCuentaNo, 1) AS Cuenta, 
  LEFT(TransDCuentaNo, 2) AS Grupo, 
  LEFT(TransDCuentaNo, 3) AS SubGrupo, 
  LEFT(TransDCuentaNo, 4) AS SubCuenta, 
  SUM(TransDDebito) AS Debito, 
  SUM(TransDCredito) AS Credito 
FROM 
  tbl_interfascontabletransacciones AS t, 
  tbl_interfasecontabletransaccionesdetalles AS td 
WHERE 
  t.TransID = td.TransDIDTrans 
  AND DATE(t.TransFechaMovimiento) BETWEEN fechaDesde 
  AND fechaHasta 
GROUP BY 
  TransDCuentaNo;
DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' 
...
OPEN cur1;
REPEAT FETCH cur1 INTO VarCuenta, 
VarGrupo, 
VarSubGrupo, 
VarSubCuenta, 
VarDebito, 
...
UPDATE 
  tbl_catalogo AS c, 
  tbl_catalogo_grupo AS cg 
SET 
  c.Balance = c.Balance +(VarCredito - VarDebito), 
  c.Debito = c.Debito + VarDebito, 
  c.Credito = c.Credito + VarCredito 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 2;
UPDATE 
  tbl_catalogo AS c, 
  tbl_catalogo_grupo AS cg 
SET 
  c.Balance = c.Balance +(VarDebito - VarCredito), 
  c.Debito = c.Debito + VarDebito, 
  c.Credito = c.Credito + VarCredito 
WHERE 
  c.Grupo = cg.ID 
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_controlesBceAnt`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_controlesBceAnt`;
CREATE PROCEDURE `cont_actualizar_cta_controlesBceAnt`(
  IN `fechaDesde` date, IN `fechaHasta` date
) BEGIN DECLARE done INT DEFAULT 0;
DECLARE VarCuenta CHAR(50);
DECLARE VarGrupo CHAR(50);
...
WHERE 
  t.TransID = td.TransDIDTrans 
  AND t.TransFechaMovimiento BETWEEN fechaDesde 
  AND fechaHasta 
GROUP BY 
  TransDCuentaNo;
DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' 
...
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.BalanceAnterior = c.BalanceAnterior +(VarCredito - VarDebito) 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 2;
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.BalanceAnterior = c.BalanceAnterior +(VarDebito - VarCredito) 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 1;
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_controles_a_cero`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_controles_a_cero`;
CREATE PROCEDURE `cont_actualizar_cta_controles_a_cero`() BEGIN 
update 
  tbl_catalogo 
set 
  Balance = 0, 
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_detalles`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_detalles`;
CREATE PROCEDURE `cont_actualizar_cta_detalles`(
  IN `fechaDesde` date, IN `fechaHasta` date
) BEGIN DECLARE done INT DEFAULT 0;
DECLARE VarCuenta CHAR(50);
DECLARE VarDebito DECIMAL(12, 2);
...
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.Balance =(VarDebito - VarCredito), 
  c.Debito = VarDebito, 
  c.Credito = VarCredito 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 1;
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.Balance =(VarCredito - VarDebito), 
  c.Debito = VarDebito, 
  c.Credito = VarCredito 
WHERE 
  c.Grupo = cg.ID 
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_detallesBceAnt`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_detallesBceAnt`;
CREATE  PROCEDURE `cont_actualizar_cta_detallesBceAnt`(
  IN `fechaDesde` date, IN `fechaHasta` date
) BEGIN DECLARE done INT DEFAULT 0;
DECLARE VarCuenta CHAR(50);
DECLARE VarDebito DECIMAL(12, 2);
...
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.BalanceAnterior =(VarDebito - VarCredito) 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 1;
UPDATE 
  ...
  tbl_catalogo_grupo AS cg 
SET 
  c.BalanceAnterior =(VarCredito - VarDebito) 
WHERE 
  c.Grupo = cg.ID 
  AND c.CuentaNo = VarCuenta 
  AND cg.Origen = 2;
...
END;
-- ----------------------------
--  Procedure definition for `cont_actualizar_cta_detalles_a_cero`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_actualizar_cta_detalles_a_cero`;
CREATE PROCEDURE `cont_actualizar_cta_detalles_a_cero`() BEGIN 
UPDATE 
  tbl_catalogo 
SET 
  BalanceAnterior = 0, 
...
END;
-- ----------------------------
--  Procedure definition for `cont_delete_trans`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_delete_trans`;
CREATE PROCEDURE `cont_delete_trans`(
  IN IdTrans INT(11)
) BEGIN 
DELETE from 
  tbl_interfascontabletransacciones 
where 
  TransID = IdTrans;
delete from 
  tbl_interfasecontabletransaccionesdetalles 
where 
  TransDIDTrans = IdTrans;
END;
-- ----------------------------
--  Procedure definition for `cont_insert_trans`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_insert_trans`;
CREATE PROCEDURE `cont_insert_trans`(
  in TransFechaMovimiento date, 
  in TransDescripcion varchar(250), 
  in TransIDTipo int(11), 
  in TransUsuario varchar(25), 
  in TransIDDoc int(11), 
  in TransDoc varchar(50), 
  in TransDocDetalle varchar(255)
) BEGIN INSERT INTO tbl_interfascontabletransacciones (
  TransFecha, TransFechaMovimiento, 
  TransDescripcion, TransIDTipo, TransUsuario, 
  TransIDDoc, TransDoc, TransDocDetalle
) 
VALUES 
  (
    Now(), TransFechaMovimiento, TransDescripcion, 
    TransIDTipo, TransUsuario, TransIDDoc, 
    TransDoc, TransDocDetalle
  );
END;
-- ----------------------------
--  Procedure definition for `cont_insert_trans_detalle`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `cont_insert_trans_detalle`;
CREATE PROCEDURE `cont_insert_trans_detalle`(
  in TransDIDTrans Int(11), 
  in TransDCuentaNo Varchar(15), 
  in TransDDescripcionCuenta Varchar(80), 
  in TransDDebito Decimal(12, 2), 
  in TransDCredito Decimal(12, 2)
) BEGIN INSERT INTO tbl_interfasecontabletransaccionesdetalles (
  TransDIDTrans, TransDCuentaNo, TransDDescripcionCuenta, 
  TransDDebito, TransDCredito, TransDBalance
) 
VALUES 
  (
    TransDIDTrans, TransDCuentaNo, TransDDescripcionCuenta, 
    TransDDebito, TransDCredito, 0
  );
END;
-- ----------------------------
--  Procedure definition for `fn_ultimo_id`
-- ----------------------------
DROP 
  FUNCTION IF EXISTS `fn_ultimo_id`;
CREATE FUNCTION `fn_ultimo_id`(
  campo varchar(15)
) RETURNS int(11) BEGIN 
SET 
  @idPrimary = 1;
SELECT 
  MAX(campo) INTO @idPrimary 
FROM 
  tbl_clientes;
RETURN @idPrimary;
END;
-- ----------------------------
--  Procedure definition for `Imprimir_Libretas`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `set_bce_banco`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `set_bce_banco`;
CREATE PROCEDURE `set_bce_banco`(
  idBanco Varchar(10)
) BEGIN DECLARE dr DOUBLE(12, 2);
...
END;
-- ----------------------------
--  Procedure definition for `set_bce_cliente`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `set_bce_cliente`;
CREATE PROCEDURE `set_bce_cliente`(
  id_cliente INT(10)
) BEGIN DECLARE dr DOUBLE(12, 2);
DECLARE cr DOUBLE(12, 2);
SELECT 
...
END;
-- ----------------------------
--  Procedure definition for `set_bce_prestamos`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `set_bce_prestamos`;
CREATE PROCEDURE `set_bce_prestamos`(
  id_prestamo INT(10)
) BEGIN DECLARE dr DOUBLE(12, 2);
DECLARE cr DOUBLE(12, 2);
SELECT 
  IFNULL(
    SUM(fld_capital_cuota_balance), 
    0
  ) INTO dr 
FROM 
  tbl_cuotas 
WHERE 
  fld_id_del_prestamo = id_prestamo;
SELECT 
  IFNULL(
    SUM(fld_interes_cuota_balance), 
    0
  ) INTO cr 
FROM 
  tbl_cuotas 
...
SET 
  fld_balance =(dr + cr) 
WHERE 
  fld_id_prestamos = id_prestamo;
END;
-- ----------------------------
--  Procedure definition for `set_bce_proveedor`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `set_bce_proveedor`;
CREATE PROCEDURE `set_bce_proveedor`(
  id_prove INT(10)
) BEGIN DECLARE dr DOUBLE(12, 2);
DECLARE cr DOUBLE(12, 2);
SELECT 
...
FROM 
  tbl_estado_proveedor 
WHERE 
  fld_proveedor_id = id_prove;
SELECT 
  IFNULL(
    SUM(fld_cr), 
    0
...
SET 
  fldBalance =(cr - dr) 
WHERE 
  fldSupplierID = id_prove;
END;
-- ----------------------------
--  Procedure definition for `set_estado_cta_cliente`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `set_estado_cta_cliente`;
CREATE PROCEDURE `set_estado_cta_cliente`(
  id_cliente INT(10)
) BEGIN TRUNCATE tbl_estado_clientes;
INSERT INTO tbl_estado_clientes (
  fld_date, fld_description, fld_dr, 
  fld_cr, fld_cliente_id, fld_tipo_doc, 
  fld_tipo_doc_id
) (
  SELECT 
    fld_fecha_entrega, 
  ...
  FROM 
    tbl_prestamos 
  WHERE 
    fld_id_del_cliente = id_cliente 
    AND fld_status_prestamos = 'Nuevo'
);
INSERT INTO tbl_estado_clientes (
  fld_date, fld_description, fld_dr, 
  fld_cr, fld_cliente_id, fld_tipo_doc, 
  ...
  SELECT 
    ...
    fld_Id_RI 
  FROM 
    tbl_recibo_ingresos_cxc 
  WHERE 
    fld_id_cliente_ri = id_cliente 
    AND fld_Estado = 'NEW'
);
INSERT INTO tbl_estado_clientes (
  fld_date, fld_description, fld_dr, 
...
INSERT INTO tbl_estado_clientes (
  ...
  FROM 
    tbl_nc_cxc 
  WHERE 
    fld_cliente_id_nc = id_cliente 
    AND fld_status_nc = 'N'
);
END;
-- ----------------------------
--  Procedure definition for `solicitudcreditogastos_flia`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `spActualizarBeneficiarios`
-- ----------------------------
DROP 
  PROCEDURE IF EXISTS `spActualizarBeneficiarios`;
CREATE PROCEDURE `spActualizarBeneficiarios`(
  IN _CodCliente INT, 
...
update 
  tbl_beneficiarios_Aportes 
set 
  NombreBeneficiario = _NombreBeneficiario, 
  ParentescoBeneficiario = _ParentescoBeneficiario, 
  CedulaBeneficiario = _CedulaBeneficiario, 
  TelBeneficiario = _TelBeneficiario, 
  CelularBeneficiario = _CelularBeneficiario, 
  WhatsappBeneficiario = _WhatsappBeneficiario, 
  NombreBeneficiario2 = _NombreBeneficiario2, 
  ParentescoBeneficiario2 = _ParentescoBeneficiario2, 
  CedulaBeneficiario2 = _CedulaBeneficiario2, 
  TelBeneficiario2 = _TelBeneficiario2, 
  CelularBeneficiario2 = _CelularBeneficiario2, 
  WhatsappBeneficiario2 = _WhatsappBeneficiario2 
where 
  CodCliente = _CodCliente;
END;
-- ----------------------------
--  Procedure definition for `spActualizarSegundoTitular`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `spInSertarBeneficiarios`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `spInsertarMnuArchivo`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `spInsertarMnuBanco`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `spInsertarMnuMantenimiento`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `spInsertarSegundoTitular`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_Actualiar_socios`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `Sp_Actualizar_Roles`
-- ----------------------------
DROP 
...
End;
-- ----------------------------
--  Procedure definition for `sp_InsertarAhorro`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_Insertar_puestos`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_insertar_roles`
-- ----------------------------
DROP 
...
End;
-- ----------------------------
--  Procedure definition for `sp_Insertar_socios`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_insertar_solicitud_credito`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `sp_mmnuCxPm`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_MnuCaja`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_mnuCXC`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_reporte_CxC`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `sp_score_Cliente`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `sp_solicitudcreditoingresos_flia`
-- ----------------------------
DROP 
...
-- ----------------------------
--  Procedure definition for `sp_solicitudcreditoingresos_gastos`
-- ----------------------------
DROP 
...
END;
-- ----------------------------
--  Procedure definition for `sp_solicitudcreditonota`
-- ----------------------------
DROP 
...
End;
-- ----------------------------
--  Procedure definition for `sp_solicitudcreditopatrimonio`
-- ----------------------------
DROP 
